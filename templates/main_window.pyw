...
from modules import arqid_logic, backup_logic
...
class MainWindow:
    ...
    def create_tab_changearqid(self, master=None):
        ...
        self.create_title("Alterar instância no ArqID.txt", f1)
        self.create_frames_arqid(f1)
        self.create_check_boxes(f1)
        self.create_buttons_arqid(f1)
        self.load_paths()

    def create_tab_backupbanco(self, master=None):
        f2 = Frame(self.n)
        self.n.add(f2, text='Restaurar Backup')
        self.create_title("Restaurar backup do banco de dados", f2)
        self.create_frames_backup(f2)
        self.create_buttons_backup(f2)
        self.load_paths_backup()
...
# region Alterar ArqID
    ...
    def create_buttons_arqid(self, master=None):
    ...
    def change_arqid_click(self):
        ...
        self.mensagem['text'] = ''
        resultado = arqid_logic.alterar_arqid(
    ...
    def save_path_click(self):
        ...
        arqid_logic.save_settings(path, server, station)
...
# region Backup Banco
    ...
    def create_frames_backup(self, master=None):
        """Criando os frames(txtedits) da tela de backup"""
    ...
    def create_buttons_backup(self, master=None):
        """Criando os botoes da tela de backup"""
        ...
        self.txt_status_backup = Label(self.txt_container,
                                       text="Status")
        self.txt_status_backup["font"] = ("Arial", "9", "bold")
        self.txt_status_backup.pack(side='top')

        self.mensagem_backup = Label(self.txt_container,
                                     text="",
                                     foreground="green")
        self.mensagem_backup["font"] = ("Arial", "9", "bold")
        self.mensagem_backup.pack(anchor="center")

        self.btn_save_path = Button(self.btn_container,
                                    text="Salvar Path",
                                    width=12,
                                    command=self.save_path_backup_click)
        self.btn_save_path.pack(side='left')

        self.btn_change_arqid = Button(self.btn_container,
                                       text="Fazer Backup",
                                       width=12,
                                       command=self.make_backup_click)
        self.btn_change_arqid.pack(side='right')

    def make_backup_click(self):
        """Funcao do botao Alterar ArqID ao ser clicado"""
        self.mensagem_backup['text'] = ''
    ...
    def save_path_backup_click(self):
        """Funcao para salvar os caminhos em um banco de dados"""
        origem = self.path_origem.get()
        destino = self.path_destino.get()

        backup_logic.save_path_backup(origem, destino)

        self.mensagem_backup['text'] = 'Informações salvas'
        self.mensagem_backup["foreground"] = "red"
        importlib.reload(backup_logic)

    def load_paths_backup(self):
        """Retorna os dados do banco para mostrar na tela"""
        row = backup_logic.get_settings()
